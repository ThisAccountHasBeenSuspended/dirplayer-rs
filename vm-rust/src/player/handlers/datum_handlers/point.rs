...
impl PointDatumHandlers {
  ...
  pub fn call(datum: &DatumRef, handler_name: &String, args: &[DatumRef]) -> Result<DatumRef, ScriptError> {
    match handler_name.as_str() {
      ...
      _ => Err(ScriptError::new(format!("No handler {handler_name} for point")))
    }
  }
  ...
  pub fn get_prop(player: &DirPlayer, datum: &DatumRef, prop: &String) -> Result<Datum, ScriptError> {
    ...
    match prop.as_str() {
      ...
      _ => {
        Err(ScriptError::new(format!("Cannot get point property {}", prop)))
      },
    }
  }
  ...
  pub fn set_prop(player: &mut DirPlayer, datum: &DatumRef, prop: &String, value_ref: &DatumRef) -> Result<(), ScriptError> {
    ...
    match prop.as_str() {
      ...
      _ => {
        Err(ScriptError::new(format!("Cannot set point property {}", prop)))
      },
    }
  }
}
