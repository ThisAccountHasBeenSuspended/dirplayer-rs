...
impl BitmapDatumHandlers {
  ...
  pub fn copy_pixels(datum: &DatumRef, args: &[DatumRef]) -> Result<DatumRef, ScriptError> {
    reserve_player_mut(|player| {
      ...
      let dest_rect = match dest_rect_or_quad {
        ...
        _ => return Err(ScriptError::new("Invalid destRect for copyPixels".to_string())),
      };
  ...
  pub fn get_prop(player: &mut DirPlayer, datum: &DatumRef, prop: &String) -> Result<DatumRef, ScriptError> {
    ...
    let bitmap = match bitmap {
      Datum::BitmapRef(bitmap) => Ok(bitmap),
      _ => Err(ScriptError::new("Cannot get prop of non-bitmap".to_string())),
    }?;
    let bitmap = player.bitmap_manager.get_bitmap(*bitmap).unwrap();
    let result = match prop.as_str() {
      ...
      _ => {
        Err(ScriptError::new(format!("Cannot get bitmap property {}", prop)))
      },
    }?;
    Ok(player.alloc_datum(result))
  }

  pub fn set_bitmap_ref_prop(player: &mut DirPlayer, bitmap_ref: BitmapRef, prop: &String, value: &DatumRef) -> Result<(), ScriptError> {
    let value = player.get_datum(value);
    match prop.as_str() {
      "paletteRef" => match value {
        Datum::Symbol(symbol) => {
          let palette = BuiltInPalette::from_symbol_string(&symbol).ok_or_else(|| ScriptError::new("Invalid built-in palette symbol".to_string()))?;
          let bitmap = player.bitmap_manager.get_bitmap_mut(bitmap_ref).unwrap();
        ...
        _ => Err(ScriptError::new(format!("Cannot set paletteRef to datum of type {}", value.type_str()))),
      },
      _ => {
        Err(ScriptError::new(format!("Cannot set bitmap property {}", prop)))
      },
    }
  }
}
