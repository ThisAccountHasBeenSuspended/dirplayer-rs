...
impl RectDatumHandlers {
  ...
  pub fn call(datum: &DatumRef, handler_name: &String, args: &[DatumRef]) -> Result<DatumRef, ScriptError> {
    match handler_name.as_str() {
      ...
      _ => Err(ScriptError::new(format!("No handler {handler_name} for rect")))
    }
  }
  ...
  pub fn get_prop(player: &DirPlayer, datum: &DatumRef, prop: &String) -> Result<Datum, ScriptError> {
    ...
    match prop.as_str() {
      "width" => {
        Ok(Datum::Int(*right as i32 - *left as i32))
      },
      "height" => {
        Ok(Datum::Int(*bottom as i32 - *top as i32))
      },
      "left" => {
        Ok(Datum::Int(*left as i32))
      },
      "top" => {
        Ok(Datum::Int(*top as i32))
      },
      "right" => {
        Ok(Datum::Int(*right as i32))
      },
      "bottom" => {
