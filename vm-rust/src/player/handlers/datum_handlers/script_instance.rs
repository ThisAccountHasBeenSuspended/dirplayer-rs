...
impl ScriptInstanceUtils {
  pub fn get_script<'a>(datum: &DatumRef, player: &'a DirPlayer) -> Result<(ScriptInstanceRef, &'a Script), ScriptError> {
  ...
  #[allow(dead_code)]
  pub fn get_instance_script_def<'a>(instance_ref: &ScriptInstanceRef, player: &'a DirPlayer) -> &'a Script {
    let script_ref = player.allocator.get_script_instance(&instance_ref).script.to_owned();
      ...
      "ancestor" => {
        ...
        match value {
          ...
          Datum::ScriptInstanceRef(ancestor_instance_id) => {
            let script_instance = player.allocator.get_script_instance_mut(&self_instance_id);
  ...
  fn get_at(datum: &DatumRef, args: &[DatumRef]) -> Result<DatumRef, ScriptError> {
    reserve_player_mut(|player| {
      ...
      match key.as_str() {
        "ancestor" => {
          let datum = player.get_datum(datum);
          let script_instance = player.allocator.get_script_instance(datum.to_script_instance_ref()?);
          Ok(player.alloc_datum(if let Some(ancestor) = &script_instance.ancestor { Datum::ScriptInstanceRef(ancestor.clone()) } else { Datum::Int(0) }))
        }
