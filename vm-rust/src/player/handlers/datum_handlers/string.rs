...
impl StringDatumUtils {
    ...
    if let Datum::String(str_val) = datum {
      match prop_name.as_str() {
        "item" | "word" | "char" | "line" => {
          ...
          Ok(Datum::StringChunk(StringChunkSource::Datum(datum_ref.clone()), chunk_expr, resolved_str))
        },
        _ => Err(ScriptError::new(format!("getPropRef: invalid prop_name {prop_name} for string")))
      }
    } else {
      return Err(ScriptError::new(format!("getPropRef: datum is not a string")));
    }
  }
  ...
  pub fn get_built_in_prop(value: &String, prop_name: &String) -> Result<Datum, ScriptError> {
    match prop_name.as_str() {
      ...
      _ => Err(ScriptError::new(format!("Invalid string built-in property {prop_name}")))
    }
  }
}
...
pub fn string_get_words(value: &String) -> Vec<String> {
  value.split_whitespace().filter(|s| !s.is_empty()).map(|s| s.to_string()).collect()
}
