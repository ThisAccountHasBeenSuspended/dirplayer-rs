...
impl BasicListChunk {
  ...
  pub fn read_offset_table(
    ...
    let offset_table = (0..offset_table_len).map(|_| reader.read_u32().unwrap() as usize).collect();
  ...
  pub fn read_items(reader: &mut BinaryReader, dir_version: u16, header: usize, offset_table: &[usize]) -> Result<Vec<Vec<u8>>, String> {
    ...
    let items = (0..offset_table.len()).map(|i| {
      let offset = offset_table[i];
      let next_offset = if i == offset_table.len() - 1 { items_len as usize } else { offset_table[i + 1] };
